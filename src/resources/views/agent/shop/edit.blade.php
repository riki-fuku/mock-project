            ...
            <div class="grid md:grid-cols-2">
                <div class="w-11/12 lg:w-10/12 mx-auto">
                    <div class="flex">
                        ...
                        <h3 class="ml-2 font-black text-2xl leading-10">{{ $shopReservation['shop']['name'] }}</h3>
                    </div>

                    <img src="{{ asset('storage/image/shop/' . (string) $shopReservation['shop']['id'] . '/top.jpeg') }}"
                        class="mt-6 w-full h-1/2 object-cover" alt="sample" />

                    <div class="flex mt-6">
                        <p>#{{ $shopReservation['shop']['shop_area']['area_name'] }}</p>
                        <p class="ml-2">#{{ $shopReservation['shop']['shop_genre']['genre_name'] }}</p>
                    </div>

                    <div class="mt-6">
                        {{ $shopReservation['shop']['description'] }}
                    </div>

                </div>

                <div class="w-11/12 lg:w-full mx-auto bg-blue-500 rounded-md relative shadow-lg shadow-gray-400 z-0">
                    <form action="{{ route('shop_update_confirm') }}" method="POST">
                        @csrf
                        <div class="px-4 py-6">
                            <div class="ml-2 font-black text-2xl leading-10 text-white">予約変更</div>
                            ...
                            <div class="mt-4">
                                ...
                                <input type="date" name="reservation_date" class="block px-3 py-1 rounded"
                                    value="{{ $shopReservation['reservation_date'] }}" min="{{ date('Y-m-d') }}"
                                    ...
                                    @for ($hour = 12; $hour < 22; $hour++)
                                        @for ($minute = 0; $minute < 60; $minute += 15)
                                            <option
                                                ...
                                                {{ $shopReservation['reservation_time'] == str_pad($hour, 2, '0', STR_PAD_LEFT) . ':' . str_pad($minute, 2, '0', STR_PAD_LEFT) . ':00' ? 'selected' : '' }}>
                                ...
                                <select name="party_size" class="block px-3 py-1 rounded mt-2 w-full"
                                    ...
                                    @for ($i = 1; $i <= 4; $i++)
                                        <option value="{{ $i }}"
                                            {{ $shopReservation['party_size'] == $i ? 'selected' : '' }}>
                                            {{ $i }}人</option>
                                    @endfor
                                </select>
                                ...
                                {{-- 店舗ID --}}
                                <input type="hidden" name="shop_id" value="{{ $shopReservation['shop']['id'] }}">

                                {{-- 店舗名 --}}
                                <input type="hidden" name="shop_name" value="{{ $shopReservation['shop']['name'] }}">
                            ...
                            <div class="mt-4 px-4 py-4 bg-blue-400 rounded-md">
                                <table class="text-left text-white">
                                    <tr>
                                        ...
                                        <td>{{ $shopReservation['shop']['name'] }}</td>
                                    </tr>
                                    <tr>
                                        <th>Date</th>
                                        <td id="reservation_date">{{ $shopReservation['reservation_date'] }}</td>
                                    </tr>
                                    <tr>
                                        <th>Time</th>
                                        <td id="reservation_time">
                                            {{ substr($shopReservation['reservation_time'], 0, -3) }}</td>
                                    </tr>
                                    <tr>
                                        <th>Number</th>
                                        <td id="party_size">{{ $shopReservation['party_size'] }}人</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
